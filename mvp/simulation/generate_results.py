...
==============================
Runs all 5 scenarios x 19 modes (95 episodes: 8 canonical + 11 §4.7
sensitivity ablations), computes per-run metrics, and saves CSV summary
tables to mvp/simulation/results/.
...
def run_episode(
    ...
    # rho_trace so downstream consumers (fig 2 panel b) can compare
    # the policy-responsive retail-pool quality to the exogenous
    # environmental Arrhenius trace. The capacity-constraint
    # infrastructure was retired in 2026-04 along with the
    # capacity-constrained RLE variant.
    ...
    for idx in range(n):
        ...
        # --- §5.8 H2 channel-attribution decision ingredients ---
        # Observer-only fields recorded by the instrumented policy
        # (action_selection.select_action ``out`` + coordinator per-channel
        # split). They let aggregate_channel_attribution.py reconstruct the
        # argmax under {none, mcp_only, pirag_only, full} context exactly,
        # without re-running the simulator. None on steps that bypass the
        # modifier (static path, cyber-outage Bernoulli reroute).
        def _veclist(name):
        ...
        decision_ledger.append({
            ...
            # §5.8 H2 channel-attribution ingredients (observer-only).
            "base_logits": (
