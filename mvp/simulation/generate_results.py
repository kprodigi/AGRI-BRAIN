...
def run_episode(
    ...
    rle_realistic = rle_tracker.rle_realistic
    # Capacity-constrained RLE uses realised actions (post BatchInventory
    # tier-capacity / fallback resolution) so it reflects operational
    # throughput rather than just policy intent. See resilience.py for
    # the match-quality + fallback semantics.
    action_names_for_rle = [ACTIONS[a] for a in action_trace]
    rle_capacity_constrained = compute_rle_capacity_constrained(
        rho_trace, action_names_for_rle, realized_action_trace,
    )
    ...
    result = {
        ...
        # weighted RLE, severity-aware match-quality RLE, and the
        # capacity-constrained variant that uses realised (post-fallback)
        # actions. Sen-welfare equity. Grounding lives in the
        # resilience.py module docstring; these populate the
        # robustness-table cells when generated by aggregate_seeds.
        # 2026-04: paper headline migrated from binary "rle" to
        # "rle_realistic" because the binary form saturates at 1.0
        # for any policy that always reroutes; rle_realistic and
        # rle_capacity_constrained do not saturate trivially.
        "ari_geom": float(np.mean(ari_geom_vals)),
        "rle_weighted": float(rle_weighted),
        "rle_realistic": float(rle_realistic),
        "rle_capacity_constrained": float(rle_capacity_constrained),
    ...
    for scenario in SCENARIOS:
        ...
        for mode in MODES:
            ...
            for iter_idx in range(n_iter):
                ...
                if n_iter > 1:
                    ...
                    trajectory_cache.setdefault(mode, []).append({
                        ...
                        "ari": episode["ari"],
                        "waste": episode["waste"],
                        # Headline RLE in the trajectory cache is the
                        # realistic form; binary preserved for legacy.
                        "rle": episode.get("rle_realistic", episode["rle"]),
                        "rle_binary": episode["rle"],
                        "rle_capacity_constrained": episode.get(
                            "rle_capacity_constrained",
                            episode.get("rle_realistic", episode["rle"])
                        ),
                        "slca": episode["slca"],
            ...
            print(f"  [{scenario:>20s}] [{mode:>17s}]{tag} ARI={episode['ari']:.3f}  "
                  ...
                  f"RLE={episode['rle_realistic']:.3f}  "
    ...
    for scenario in SCENARIOS:
        for method in table1_methods:
            ...
            table1_rows.append({
                ...
                "ARI": round(ep["ari"], 3),
                # Headline RLE migrated from the saturating binary
                # form to the severity-aware match-quality form
                # (resilience.compute_rle_realistic). The legacy
                # binary value is preserved as RLE_binary, the
                # capacity-constrained operational variant as
                # RLE_capacity_constrained, and the EU-hierarchy
                # robustness variant as RLE_weighted.
                "RLE": round(ep["rle_realistic"], 3),
                "RLE_binary": round(ep["rle"], 3),
                "RLE_weighted": round(ep["rle_weighted"], 3),
                "RLE_capacity_constrained": round(ep["rle_capacity_constrained"], 3),
    ...
    for scenario in SCENARIOS:
        ...
        for mode in MODES:
            ...
            summary[scenario][mode] = {
                "ari": round(ep["ari"], 4),
                # Headline RLE = realistic match-quality. Legacy
                # binary form preserved as rle_binary for backward
                # compatibility with consumers reading older summary
                # JSON dumps.
                "rle": round(ep["rle_realistic"], 4),
                "rle_binary": round(ep["rle"], 4),
                "rle_weighted": round(ep["rle_weighted"], 4),
                "rle_capacity_constrained": round(ep["rle_capacity_constrained"], 4),
