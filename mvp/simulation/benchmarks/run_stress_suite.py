...
def _perturb_df(df: pd.DataFrame, stressor: str, rng: np.random.Generator) -> pd.DataFrame:
    ...
    cell, so the H3 robustness pass-rate of 40/40 was technically true
    ...
    -0.10 H3 bound a meaningful test. We also expand the stressor set
...
def main() -> None:
    ...
    # seed=42 only (n=1) which left the H3 robustness pass-rate without
