...
dataset, second crop, or second region).

2026-06 rename: the script and its artifacts (`temporal_stability_*.
{json,csv}`) were renamed from the former `external_validity_*` names,
which mislabelled the construct. There is no remaining external-validity
artifact in the repo; genuine out-of-distribution validation (a held-out
trace/crop/region) is tracked separately as a future item.
"""
...
def main() -> None:
    ...
    (RESULTS_DIR / "temporal_stability_summary.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8"
    )
    pd.DataFrame(rows).to_csv(RESULTS_DIR / "temporal_stability_summary.csv", index=False)
    pd.DataFrame(delta_rows).to_csv(RESULTS_DIR / "temporal_stability_deltas.csv", index=False)
    print(f"Saved {RESULTS_DIR / 'temporal_stability_summary.json'}")
    print(f"Saved {RESULTS_DIR / 'temporal_stability_summary.csv'}")
    print(f"Saved {RESULTS_DIR / 'temporal_stability_deltas.csv'}")
