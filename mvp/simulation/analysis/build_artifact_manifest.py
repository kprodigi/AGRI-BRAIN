...
def _is_canonical_path(rel: str, include_raw: bool, run_tag: str) -> bool:
...
def main() -> None:
    ...
    for p in sorted(RESULTS_DIR.rglob("*")):
        ...
        if not _is_canonical_path(rel, include_raw, run_tag):
            continue
    ...
    if retired_overrides:
        raise RuntimeError(
            ...
            + ", ".join(retired_overrides)
            + " and build the manifest inside the fresh run's source snapshot"
        )
    declared_commit = os.environ.get("AGRIBRAIN_GIT_COMMIT", "").strip()
    if declared_commit and head_sha and declared_commit != head_sha:
        raise RuntimeError(
