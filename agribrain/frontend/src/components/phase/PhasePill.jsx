...
const PHASE_META = {
  monitoring: {
    ...
    dot: "bg-blue-500",
    desc: "Recommendation preview only. The decision is shown but not added to the decision log or submitted to the optional chain logger.",
  },
  advisory: {
    label: "Advisory",
    short: "ADV",
    ...
    dot: "bg-amber-500",
    desc: "Decisions await operator review. Approval finalizes and records them; chain logging is conditional on configuration and successful submission.",
  },
  autonomous: {
    label: "Auto-finalize (development)",
    short: "AUTO",
    bg: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200",
    dot: "bg-emerald-500",
    desc: "Decisions are finalized and recorded immediately; chain logging is conditional on configuration and successful submission.",
  },
};
...
export default function PhasePill() {
  ...
  const meta = PHASE_META[runtimePhaseOrDefault(phase)];
    ...
    <Tooltip>
      ...
      <TooltipContent>
        <div className="max-w-xs text-xs">
          <div className="font-semibold mb-1">Development runtime phase: {meta.label}</div>
          <div>{meta.desc}</div>
          {phase === "advisory" && (
