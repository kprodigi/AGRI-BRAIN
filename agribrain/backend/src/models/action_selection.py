...
preference (so the no_slca ablation still shows a measurable H2 gap)
...
# (the runtime validation run on commit 1d9caf0 showed MCP-only retrieval
...
def select_action(
    ...
    if context_modifier is not None:
        ...
        if out is not None:
            ...
            # Observer-only side-channel for exact offline counterfactual
            # channel attribution (the §5.8 H2 mechanism analysis). None of
            ...
            # amplification coefficient ``slca_amp``, plus the per-channel
            # modifier components the coordinator records, the H2 aggregator
            # reconstructs argmax(base + m + slca_boost(m)) under the four
            # context configurations {none, mcp_only, pirag_only, full}
            # exactly the way the policy would have, including the governance
            # override condition.
            _amp_obs = slca_amp_coeff if slca_amp_coeff is not None else 0.40
            out["base_logits"] = [float(v) for v in logits]
            out["slca_shaping"] = [float(v) for v in (_slca_bonus + _slca_rho_bonus * rho)]
            out["slca_amp"] = float(_amp_obs)
