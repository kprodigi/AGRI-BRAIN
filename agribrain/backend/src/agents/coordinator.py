...
class AgentCoordinator:
    ...
    def __init__(
        ...
        # Observer-only decision ingredients for the §5.8 H2 channel
        # attribution. ``_step_base_logits`` is the pre-context logit vector;
        # ``_step_modifier_mcp`` / ``_step_modifier_pirag`` are the MCP-only
        # and piRAG-only feature-masked modifier components (same learned
        # Theta_context, temporal modulation, and cooperative blend the live
        # modifier received). Combined they let the aggregator reconstruct
        # argmax under {none, mcp_only, pirag_only, full} context exactly,
        # with no re-dispatch and no RNG consumption.
    ...
    def _compute_step_context(
        ...
        try:
            ...
            # ---- Per-channel modifier attribution (observer-only) ----
            # Recompute the modifier under the MCP-only and piRAG-only feature
            # masks, reusing the already-dispatched primary and cooperative
            # results (cheap matrix ops -- no re-dispatch, no RNG draw). With
            # the full live modifier these give an exact additive channel
            # split that the §5.8 H2 aggregator turns into per-channel decision
            # counterfactuals. The same cooperative blend / veto applied to the
            # live modifier is mirrored here so the split stays faithful on the
            # 12-30 h overlay window. The governance veto is compliance-driven,
            # so it is attributed to the MCP channel only (a "if only piRAG were
            # present" counterfactual would not trip a compliance veto).
            ...
            try:
                ...
                _log.debug("per-channel modifier attribution skipped: %s", _exc)
                self._step_modifier_mcp = None
                self._step_modifier_pirag = None
