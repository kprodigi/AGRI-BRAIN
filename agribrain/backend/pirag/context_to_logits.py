...
OVER_STEER: bool = False
"""Ablation flag (#1b over-steering). When True, ``compute_context_modifier``
bypasses the retrieval-quality guard (and the physics / empty-input gates) and
skips the per-element magnitude clip, so the context modifier is applied on
EVERY decision at full strength instead of on the guard-selected ~25%. This
tests whether the conservative gated+capped design is optimal versus an
always-on, uncapped one. Production default is False (zero runtime impact);
only the over-steering ablation runner sets it True."""
...
channel alone). With SLCA_BONUS since softened to the live
`action_selection.SLCA_BONUS` value [-0.05, +0.10, +0.05] (+0.15-logit
advantage), the ORIGINAL THETA_CONTEXT values produced an
...
def compute_context_modifier(
    ...
    # OVER_STEER bypasses the guard so the modifier applies on every step.
    if not OVER_STEER and not rag_context.get("guards_passed", True):
        return np.zeros(3)
    ...
    if not OVER_STEER and physics_gate_enabled and physics_score < 0.03:
        return np.zeros(3)

    if not OVER_STEER and not mcp_results and not rag_context:
        return np.zeros(3)

    psi = extract_context_features(mcp_results, rag_context, obs)

    # Single-channel ablation modes (mcp_only / pirag_only). The
    # structural gating in coordinator._compute_step_context (commit
    # 1d9caf0) already produces channel-level differentiation by
    ...
    # very ablation difference being claimed) now that the structural
    # gating provides genuine channel-level differentiation. The
    # runtime validation run on heatwave shows the mask + structural
    # gating alone produce ~8pp action-distribution divergence
    # between mcp_only and pirag_only, which is well above noise.
    ...
    if not OVER_STEER and physics_gate_enabled:
        modifier *= max(0.0, min(1.0, physics_score / 0.15))
    if not OVER_STEER:
        # OVER_STEER removes the per-element magnitude cap (the "remove cap_frac"
        # half of the ablation); gated production always clips to +/-1.0.
