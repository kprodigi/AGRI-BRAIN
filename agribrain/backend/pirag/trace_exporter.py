...
class TraceExporter:
    ...
    def export_interoperability_trace(self) -> List[Dict]:
        ...
        scenarios surface in the bytes), and a small ``run_signature``
        field summarising the trace contents so two equally-prefixed
        runs cannot collide.
        """
        traces = []
        for t in self._traces[:5]:
            ...
            step_traces.append({
                "request": {
                    ...
                    "params": {
                        "uri": f"agribrain://quality/spoilage_risk?scenario={t.scenario}&hour={t.hour}",
                    },
                },
            ...
            traces.append({
                ...
                "hour": float(t.hour),
                "role": t.role,
                # Make the numeric state explicit so per-scenario
                # differences surface in the persisted bytes.
                "state_at_step": {
                    "rho": float(t.rho),
                    "temperature": float(t.temperature),
                    "humidity": float(t.humidity),
