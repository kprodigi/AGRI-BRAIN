...
def test_bca_fallback_rate_zero_on_real_variance():
    """Guard: the *honest* BCa fallback rate is 0 on real-variance input,
    and zero-variance cells are counted as ``deterministic_cells`` (BCa
    mathematically undefined) rather than inflating the fallback rate.

    This pins the post-2026-05 honest split the methods section quotes:
    ``bca_fallback_rate = bca_fallbacks / bca_calls`` excludes the
    deterministic cells where BCa was never going to apply."""
    SIM_BENCH = Path(__file__).resolve().parents[3] / "mvp" / "simulation" / "benchmarks"
    sys.path.insert(0, str(SIM_BENCH))
    from aggregate_seeds import (
        bootstrap_ci, _reset_bca_fallback_stats, _bca_fallback_stats_snapshot,
    )
    import numpy as np

    _reset_bca_fallback_stats()
    rng = np.random.default_rng(20260605)
    # 12 genuinely-variable cells (n=20, skewed log-normal -> exactly the
    # regime BCa exists for). Every one must receive a real BCa interval.
    for i in range(12):
        vals = rng.lognormal(mean=0.0, sigma=0.6, size=20).tolist()
        lo, hi = bootstrap_ci(vals, n_boot=500, cell_key=("real", i))
        assert np.isfinite(lo) and np.isfinite(hi) and lo <= hi, (
            f"cell {i}: degenerate BCa interval ({lo}, {hi}) on variance>0 input"
        )
    # 3 deterministic (zero-variance) cells: BCa undefined -> short-circuit.
    for j in range(3):
        bootstrap_ci([7.0] * 20, n_boot=500, cell_key=("det", j))

    stats = _bca_fallback_stats_snapshot()
    # The honest rate must be exactly 0: no variance>0 cell fell back.
    assert stats["bca_calls"] == 12, f"expected 12 BCa calls; got {stats}"
    assert stats["bca_fallbacks"] == 0, f"true BCa fallbacks must be 0; got {stats}"
    assert stats["bca_fallback_rate"] == 0.0, (
        f"honest bca_fallback_rate must be 0.0; got {stats['bca_fallback_rate']}"
    )
    # Deterministic cells are tracked separately, NOT as fallbacks.
    assert stats["deterministic_cells"] == 3, (
        f"expected 3 deterministic cells; got {stats}"
    )
