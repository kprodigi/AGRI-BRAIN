...
**Run identity:** report the run tag and SHA-256 manifest from
`mvp/simulation/results/artifact_manifest.json`, plus its simulation source
commit and publication code commit. The commits are equal for fresh evidence
and distinct only for authorized dual-provenance publication recovery.
...
That command is the normal fresh-run path. When completed methodology-aligned
workers are byte-preserved but their original publishers failed, do not rerun
the arrays; use only the fail-closed procedure in
`docs/PUBLICATION_RECOVERY.md`.
...
The manifest stores each generated artifact's SHA-256 plus the simulation
source and publication code identities. Those identities are the same for a
fresh run; authorized recovery additionally binds their distinct values to the
canonical recovery receipt and records `simulation_rerun: false`. The tracked
`agribrain/backend/requirements-lock.txt` is consumed by the publication job;
it is not generated by that job. Report the actual Python version, platform,
installed packages, canonical environment values, and lockfile hash from
`publication_environment.json`, not from development-machine examples or the
lockfile header alone.
