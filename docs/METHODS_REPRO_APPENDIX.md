...
**Project:** AGRI-BRAIN  
**Repository:** https://github.com/kprodigi/AGRI-BRAIN (canonical)  
...
5. `benchmarks/run_temporal_stability.py`
...
- Time-window stability check produced by `benchmarks/run_temporal_stability.py`.
- Outputs (renamed 2026-06 from `external_validity_*`, which mislabelled the construct):
...
- OS: Windows 11 (local); SLES on the HPC cluster.
...
Every manuscript claim is backed by a committed artifact under
`mvp/simulation/results/`. `artifact_manifest.json` records the SHA-256
and the exact `git_commit` for each one, so any cited number can be
traced to a hashed file and the commit that produced it.
