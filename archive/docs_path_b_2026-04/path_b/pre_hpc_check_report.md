...
| 2.3 | SLURM script sanity | PASS | `--time=24:00:00`, `--mem=32G`, `--cpus-per-task=8`; Stage 4 loops `run_single_seed.py` across 20 seeds; all 9 MODES and 5 SCENARIOS are iterated inside `run_all()` / `run_single_seed.py:34-35` including `no_yield`. Note that Stage 3 (`run_benchmark_suite.py`) only records 5 of the 9 modes (line 124); the full 9-mode coverage comes from Stage 4 |
