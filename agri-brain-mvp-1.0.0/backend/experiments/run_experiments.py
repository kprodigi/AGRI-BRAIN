...
def run_single(df_base: pd.DataFrame, policy: Policy, distance_mult: float, seed: int) -> Dict:
    ...
    # Activity-based emission model (GHG Protocol, WRI/WBCSD, 2004):
    #   E_transport = distance * EF_vehicle
    mean_carbon = float(np.mean(carbons))
    # SLCA carbon component: C = 1 / (1 + carbon/cap) (monotone decreasing)
    slca = 1.0 / (1.0 + mean_carbon / 100.0)
